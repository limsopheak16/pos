...
const {token} = useContext(AppInfoContext);
  ...
  const handleDelete = async () => {
    try {
      ...
      if (response.ok) {
        ...
        router.push("/supplier"); // Redirect to the suppliers list
      } else {
        alert("Delete failed!");
      }
    } catch (error) {
  ...
  return (
    <div className="flex justify-center items-center min-h-screen bg-gray-100">
      <form className="bg-white p-8 rounded-lg shadow-lg w-full max-w-4xl space-y-6">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            <label htmlFor="taxIdentification" className="block font-medium">Tax Identification</label>
            <input
              type="text"
          ...
          <div>
            ...
            {formData.imageUrl ? (
              <div className="mt-2">
                <img src={formData.imageUrl} alt="Profile" className="w-full h-48 object-cover rounded" />
              </div>
            ) : (
        ...
        <div className="mt-4 space-x-4 flex justify-start">
          ...
          <Button type="button" onClick={handleUpdate} className="p-2 bg-yellow-500 text-white rounded">
            Update
          </Button>
